...
int             kfork(void);
int             growproc(int);
int             rlimit_as_ok(struct proc *, uint64); // RLIMIT_AS check (POSIX setrlimit)
void            cpuacct_tick(struct proc *);          // RLIMIT_CPU charge (POSIX setrlimit)
int             capable(int);                         // POSIX appropriate-privilege check (capability)
