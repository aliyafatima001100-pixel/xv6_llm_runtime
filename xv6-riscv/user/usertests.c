...
bigargtest(char *s)
{
  ...
  if(pid == 0){
    ...
    int i;
    // Each argument must be big enough that MAXARG-1 of them cannot fit in the
    // user stack exec builds, which is USERSTACK pages. Upstream hard-codes 400
    // bytes because upstream's USERSTACK is one page; this kernel raised it to 4
    // (the RPC and inference paths overflow a single page), so 31*400 bytes fit,
    // the exec below wrongly SUCCEEDED, and this test failed. Size it from the
    // constant instead so it stays correct whatever USERSTACK becomes.
    char big[(USERSTACK*PGSIZE)/(MAXARG-1) + 256];
