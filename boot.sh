#!/usr/bin/env bash
# Boot the prebuilt single-node xv6 kernel under QEMU (root-free, user-mode net).
# The kernel is prebuilt; there is no source to compile here.
#   Set XV6_CPUS to change the core count (baseline measurement uses XV6_CPUS=1).
...
exec qemu-system-riscv64 -machine virt -bios none -kernel kernel/kernel \
  -m 256M -smp "${XV6_CPUS:-3}" -nographic -global virtio-mmio.force-legacy=false \
